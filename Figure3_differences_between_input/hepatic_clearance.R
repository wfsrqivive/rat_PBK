...
difference_between_approaches_selected <- figure_data %>%
  ...
  mutate(method.Clint= ifelse(str_detect(method.Clint, "S9"), 
                              paste0(method.Clint," ","out of 25",")"),
                       ifelse(str_detect(method.Clint, "hep"), 
                               paste0(method.Clint," ","out of 38",")"),
                              paste0(method.Clint," ","out of 44)"))))

tiff("Figure3B_Clearance.tiff", units="in", width=15, height=5, res=150)
...
pc <- ggplot(figure_data, 
  ...
  scale_y_continuous(name = "Log10 (PBK predicted Cmax /\nobserved Cmax)",
                     limits = c(-5, 5))+
  guides(color = FALSE) +
  labs(tag = "B. Intrinsic hepatic clearance") +
