...
numberWithin<-model_results %>%
  ...
  distinct(method.partition, compound, med.pred.obs.ratio, color) %>%
  group_by(method.partition,color) %>%
  tally() %>%
...
difference_between_approaches_selected <- figure_data %>%
  ...
  left_join(., numberWithin, by = "method.partition") %>%
  mutate(method.partition = paste0(method.partition," ","(",n, " ","out of 44)")) 

tiff("Figure3A_Partition.tiff", units="in", width=15, height=5, res=150)

pb <- ggplot(figure_data, 
