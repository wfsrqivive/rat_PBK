...
numberWithin<-model_results %>%
  ...
  distinct(method.Fup, compound, med.pred.obs.ratio, color) %>%
  group_by(method.Fup,color) %>%
  tally() %>%
...
difference_between_approaches_selected <- figure_data %>%
  ...
  left_join(., numberWithin, by = "method.Fup") %>%
  mutate(method.Fup = paste0(method.Fup," ","(",n)) %>%
  mutate(method.Fup= ifelse(str_detect(method.Fup, "vitro"), 
                              paste0(method.Fup," ","out of 39",")"), 
                            paste0(method.Fup," ","out of 44)")))
 # mutate(method.Fup = paste0(method.Fup," ","(",n, ")"))

tiff("Figure3C_Fup.tiff", units="in", width=15, height=5, res=150)

pa <- ggplot(figure_data, 
