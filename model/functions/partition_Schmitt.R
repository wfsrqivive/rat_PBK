#code obtained from https://github.com/metrumresearchgroup/PBPK_PC
library(tidyverse)
...
#tissue_comp_Schmitt<-read.csv("input_data/data/unified_tissue_comp.csv")
calcKp_Schmitt <- function(logP, pKa1, pKa2, pKa3, Fup, type, dat){
  ...
  logMA <- 1.294+0.304*logP  #in case we don't have a direct logMA
  ...
  W <- switch(type,
              ...
              10^(pKa1-pH),
              #4-diprotic acid
              10^(pH-pKa1)+10^(2*pH-pKa1-pKa2), 
              #5-diprotic base
              10^(pKa2-pH)+10^(pKa1+pKa2-2*pH), 
              #6-monoprotic acid monoprotic base (acid comes first)
              10^(pKa2-pH)+10^(pH-pKa1),
              #7-triprotic acid
              10^(pH-pKa1)+10^(2*pH-pKa1-pKa2)+10^(3*pH-pKa1-pKa2-pKa[3]))
  #             #8-triprotic base
  #             10^(pKa1+pKa2+pKa[3]-3*pH),
  ...
  Kp <- dat2%>%pull(Kp)#as.list(dat2$Kp)
  names(Kp) <- name
