model <- function (pars, tout, state, dosing){#input required for the solver
  ...
  derivs <- function(t, state, parms) { # returns rate of change
    with(as.list(c(state, parms)), {
      ...
      dAve = 4*D + Qad*(Cadipose/Kpad*BP) + Qbo*(Cbone/Kpbo*BP) + Qbr*(Cbrain/Kpbr*BP) +  Qhe*(Cheart/Kphe*BP) + Qki*(Ckidney/Kpki*BP) + Qh*(Cliver/Kpli*BP)  + Qmu*(Cmuscle/Kpmu*BP) + Qsk*(Cskin/Kpsk*BP) + Qte*(Ctestes/Kpte*BP) + Qre*(Crest/Kpre*BP) - Qlu*Cvenous 						
      ...
      res<-c(dAad, dAbo, dAbr, dAgu, 
             ...
             dAmu, dAsk, dAsp, dAte, 
             dAve, dAar, dAre, dD, 
             dAliClearance, dAkiClearance,dAUCplasma) 
